...
const AdminDashboard = () => {
  ...
  const menuItems = [
    ...
    { id: "campaigns", label: "Global Campaigns", icon: <BarChart3 size={20} /> },
  ];
      ...
      <div className="p-6">
        ...
        <nav className="space-y-1.5">
          {menuItems.map((item) => (
            ...
            >
              <div className="flex items-center gap-3">
                <span className={`${active === item.id ? "text-emerald-400" : "text-zinc-500 group-hover:text-zinc-300"}`}>
                  {item.icon}
                </span>
                <span className="font-medium text-sm">{item.label}</span>
              </div>
              {active === item.id && (
                <ChevronRight size={14} className="text-emerald-500 animate-in slide-in-from-left-2" />
              )}
            </button>
          ))}
        </nav>
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-zinc-200 px-4 md:px-8 flex items-center justify-between sticky top-0 z-10">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            <div className="flex items-center gap-2 md:gap-3 group cursor-pointer">
              ...
              <div className="w-9 h-9 md:w-10 md:h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg border-2 border-white">
                <User size={18} />
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto bg-[#F8FAFC]">
          <div className="p-4 md:p-8 max-w-6xl mx-auto">
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              {active === "users" && (
                <div className="bg-white rounded-2xl md:rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-zinc-200 p-4 md:p-8">
                  <AdminCreateUser />
                </div>
              )}
